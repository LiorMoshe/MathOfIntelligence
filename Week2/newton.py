'''This is the code from sirajs source repository.'''
from sympy import *
...
def nm():
    ...
    iter_n = 0
    print(xn[-1] - x_result)
    while np.linalg.norm((np.array(xn[-1]) - x_result).astype(int)) > target_precision:
        ...
        iter_n += 1
    print("NEWTON METHOD: result distance:", np.linalg.norm((np.array(xn[-1]) - 
        x_result).astype(int))) 
